...
export const PropertyItems = () => {
  ...
  const calculateTotal = (fieldName: "costPrice" | "deduction" | "amountClaimed") => {
  ...
  const totalDeduction = calculateTotal("deduction");
  const totalAmountClaimed = calculateTotal("amountClaimed");
  ...
  return (
    <>
          ...
          <TableBody>
            {fields.map((field, index) => (
              <TableRow key={field.id}>
                ...
                <TableCell>
                  <TextField<Schema>
                    name={`propertyItems.${index}.deduction`}
                    label="Deduction"
                    onChange={(e) => {
                      e.target.value = formatCurrency(e.target.value);
                    }}
      ...
      <Grid container spacing={2} sx={{ mt: 2 }}>
        ...
        <Grid size={{ xs: 4 }}>
          <Typography variant="subtitle1">
            Total Amount Claimed: ₦{totalAmountClaimed.toLocaleString()}
          </Typography>
        </Grid>
      </Grid>
    </>
