...
const Page = () => {
  return (
    <Grid container spacing={2}>
      <Grid size={{ xs: 12 }}>
        <Typography variant="h6" gutterBottom>{"Local Account Details"}</Typography>
      </Grid>
      <Grid size={{ xs: 6 }}>
        <TextField<Schema>
          name="bankName"
      ...
      <Grid size={{ xs: 12 }}>
        <Divider sx={{ my: 3 }} />
        <Typography variant="h6" gutterBottom>{"Foreign Account Details"}</Typography>
      </Grid>
      <Grid size={{ xs: 6 }}>
        <TextField<Schema>
          name="bankName2"
...
const Provider = ({ readOnly }: ProviderProps) => {
  ...
  const handleSubmit: SubmitHandler<Schema> = (data) => {
    updateFormData(data);
    navigate("/kyc/corporate-cdd/file-uploads");
  };
